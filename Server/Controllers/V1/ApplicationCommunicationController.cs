...
namespace CandidatePortal.Server.Controllers.V1
{
    ...
    public class ApplicationCommunicationController : ControllerBase
    {
        private readonly IApplicationCommunicationService _repo;
        private readonly IReturnResponseService _returnResponse;
        ReturnResponses resp = new ReturnResponses();
        private ILoggerService _logger;

        public ApplicationCommunicationController(IApplicationCommunicationService repo, IReturnResponseService returnResponse, ILoggerService logger)
        {
            _repo = repo;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] ApplicationCommunicationAddDTO item)
        {
            if (item == null)
            {
        ...
        [HttpPut]
        public async Task<IActionResult> Put([FromBody] ApplicationCommunicationAddDTO item)
        {
            if (item == null)
            {
                _logger.LogError($"Communication Model Is Null");
        ...
        public async Task<IActionResult> GetAppCommunicaionByAppsNumberAsync(int NApplicationNumber)
        {
